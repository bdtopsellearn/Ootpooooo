...
from firebase_sync import init_firebase, is_enabled as firebase_enabled, mark_dirty as firebase_mark_dirty, restore_if_needed as firebase_restore_if_needed, start_worker as firebase_start_worker, sync_sqlite_to_firestore as firebase_sync_now
...
# SQLite remains the runtime database; Firestore provides cloud persistence/restore for Render.
firebase_init_ok = init_firebase()
if firebase_init_ok:
    firebase_restore_if_needed(conn, db_lock)
    firebase_start_worker(conn, db_lock, interval=20)
...
async def periodic_firebase_sync(context: ContextTypes.DEFAULT_TYPE):
    if firebase_enabled():
        firebase_sync_now(conn, db_lock)
