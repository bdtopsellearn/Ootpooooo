...
async def fake_otp_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "fakeotp_toggle":
        ...
        if group_id:
            ...
            # Safe fallback: test only, synthetic data, delivered to the requesting admin.
            try:
                await context.bot.send_message(
                    chat_id=user_id,
                    text=fake_otp_generate_message(),
                    parse_mode="HTML"
                )
                set_bot_setting(FAKE_OTP_LAST_SENT_KEY, str(time.time()))
                ok, msg = True, "Demo OTP test sent to your private chat."
            except Exception as exc:
                ok, msg = False, f"Demo test failed: {exc}"
...
def bottom_menu_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    """Screenshot-style main dashboard. Legacy command aliases remain supported."""
    rows = [
        [KeyboardButton("📱 Get Number", style=KBS.PRIMARY, icon_custom_emoji_id=safe_icon(CUSTOM_EMOJIS.get("GET_NUMBER", "")))],
        [KeyboardButton("📊 Traffic", style=KBS.SUCCESS), KeyboardButton("🔥 Proxy/VPN", style=KBS.PRIMARY)],
        [KeyboardButton("🎁 Refer", style=KBS.PRIMARY, icon_custom_emoji_id=safe_icon(_EMOJI_REFER)), KeyboardButton("🔐 2FA", style=KBS.SUCCESS)],
        [KeyboardButton("💸 Withdrawal", style=KBS.SUCCESS)],
    ]
    if is_admin(user_id):
...
def premium_admin_keyboard():
    rows = [
        [InlineKeyboardButton("⚡ 𝐐𝐔𝐈𝐂𝐊 𝐒𝐓𝐀𝐓𝐒", callback_data="pro_dashboard", style=KBS.SUCCESS),
         InlineKeyboardButton("👥 𝐔𝐒𝐄𝐑 𝐌𝐀𝐍𝐀𝐆𝐄𝐑", callback_data="pro_users", style=KBS.PRIMARY)],
        [InlineKeyboardButton("📦 𝐒𝐓𝐎𝐂𝐊 𝐂𝐎𝐍𝐓𝐑𝐎𝐋", callback_data="admin_stock_management", style=KBS.SUCCESS),
         InlineKeyboardButton("🧩 𝐒𝐄𝐑𝐕𝐈𝐂𝐄 𝐌𝐀𝐍𝐀𝐆𝐄𝐑", callback_data="admin_service_manager", style=KBS.PRIMARY)],
        [InlineKeyboardButton("🌍 𝐂𝐎𝐔𝐍𝐓𝐑𝐘 𝐌𝐀𝐍𝐀𝐆𝐄𝐑", callback_data="admin_country_manager", style=KBS.PRIMARY),
         InlineKeyboardButton("🧪 𝐃𝐄𝐌𝐎 𝐎𝐓𝐏", callback_data="fakeotp_menu", style=KBS.SUCCESS)],
        ...
        [InlineKeyboardButton("🌐 𝐀𝐃𝐕𝐀𝐍𝐂𝐄𝐃 𝐒𝐓𝐎𝐂𝐊", callback_data="pro_stock", style=KBS.SUCCESS),
         InlineKeyboardButton("🧩 𝐀𝐃𝐕𝐀𝐍𝐂𝐄𝐃 𝐒𝐄𝐑𝐕𝐈𝐂𝐄𝐒", callback_data="pro_services", style=KBS.PRIMARY)],
        [InlineKeyboardButton("🌐 𝐀𝐃𝐕𝐀𝐍𝐂𝐄𝐃 𝐂𝐎𝐔𝐍𝐓𝐑𝐈𝐄𝐒", callback_data="pro_countries", style=KBS.PRIMARY),
         InlineKeyboardButton("📣 𝐀𝐃𝐕𝐀𝐍𝐂𝐄𝐃 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓", callback_data="pro_broadcast", style=KBS.PRIMARY)],
        [InlineKeyboardButton("☁️ 𝐅𝐈𝐑𝐄𝐁𝐀𝐒𝐄 𝐒𝐘𝐍𝐂", callback_data="pro_firebase", style=KBS.SUCCESS),
         InlineKeyboardButton("🗄️ 𝐃𝐀𝐓𝐀𝐁𝐀𝐒𝐄 𝐂𝐎𝐑𝐄", callback_data="pro_database", style=KBS.SUCCESS)],
...
# ================= SAFE USER SHORTCUT PANELS =================
async def send_proxy_vpn_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Informational Proxy/VPN panel; it does not distribute, validate, or collect proxies."""
    user_id = update.effective_user.id
    if await ban_check(update, context):
        return
    text = (
        "🔥 <b>PROXY / VPN</b>\n\n"
        "🛡 <b>Privacy & connectivity</b>\n"
        "• Use a reputable VPN provider you trust.\n"
        "• Never send passwords, API keys, or proxy credentials to the bot.\n"
        "• This panel does not test, sell, or distribute proxies.\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "⚙️ <b>Status:</b> Informational mode"
    )
    await send_clean_message(update, context, user_id, text,
                             reply_markup=bottom_menu_keyboard(user_id),
                             parse_mode="HTML", auto_delete=False)
...
async def send_2fa_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Safe 2FA information panel. No real secret keys are requested or processed."""
    user_id = update.effective_user.id
    if await ban_check(update, context):
        return
    text = (
        "🔐 <b>2FA SECURITY</b>\n\n"
        "Two-factor authentication adds a second verification step to an account.\n\n"
        "✅ Keep your authenticator secret key private.\n"
        "✅ Save recovery codes somewhere secure.\n"
        "❌ Never send a real 2FA secret or verification code to this bot.\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🛡 <b>Mode:</b> Security information only"
    )
    await send_clean_message(update, context, user_id, text,
                             reply_markup=bottom_menu_keyboard(user_id),
                             parse_mode="HTML", auto_delete=False)
...
async def handle_user_quick_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📊 Traffic":
        ...
        return True
    if text == "🔥 Proxy/VPN":
        await send_proxy_vpn_panel(update, context)
        return True
    if text == "🎁 Refer":
        await show_invite(update, context)
        return True
    if text == "🔐 2FA":
        await send_2fa_panel(update, context)
        return True
    if text == "💸 Withdrawal":
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in (BTN_GET_NUMBER, "⚡ GET NUMBER", "📱 Get Number"):
    ...
    elif text in (BTN_LIVE, "📡 LIVE TRAFFIC", "📊 Traffic"):
        await send_live_traffic_panel(update, context)
    elif text in (BTN_REFER, "🤝 REFER & EARN", "🎁 Refer"):
        await show_invite(update, context)
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(service_selection_callback, pattern=r"^svc_sel\|"))
    application.add_handler(CallbackQueryHandler(country_selection_callback, pattern=r"^cnt_sel\|"))
    application.add_handler(CallbackQueryHandler(back_to_services_callback, pattern="^back_to_services$"))
    application.add_handler(CallbackQueryHandler(country_add_service_callback, pattern=r"^cnt_add_svc\|"))
    ...
    # ── External API auto-bootstrap intentionally disabled ─────────────────────
    # Do not ship third-party API URLs/tokens in source code.
    # Existing API Manager / database entries remain available for user-managed
    # configuration. No external credential is created automatically.
    print("ℹ️ External API auto-bootstrap disabled; use Admin → Manage API for authorized configuration.")
