...
def db_exec(query, params=()):
    """Execute a write query, commit to SQLite, and mark Firebase dirty."""
    with db_lock:
...
# ================= MAIN BUTTON ON/OFF CONTROL =================
MAIN_BUTTONS = {
    "get_number": {"label": BTN_GET_NUMBER, "title": "📱 GET NUMBER"},
    "find_otp": {"label": BTN_FIND_OTP, "title": "🔎 FIND OTP"},
    "live_traffic": {"label": BTN_LIVE, "title": "📊 LIVE TRAFFIC"},
    "balance": {"label": BTN_BALANCE, "title": "💰 BALANCE"},
    "refer": {"label": BTN_REFER, "title": "🎁 REFER & EARN"},
    "contact": {"label": BTN_CONTACT, "title": "📞 CONTACT"},
    "proxy": {"label": BTN_PROXY, "title": "🔥 PROXY/VPN"},
    "admin": {"label": BTN_ADMIN, "title": "🛡 ADMIN PANEL"},
}

def main_button_enabled(key: str) -> bool:
    return bool(get_setting(f"main_button_{key}", "1") not in ("0", 0, False, "false", "False"))

def set_main_button_enabled(key: str, enabled: bool):
    update_setting(f"main_button_{key}", "1" if enabled else "0")

def main_button_control_keyboard() -> InlineKeyboardMarkup:
    rows = []
    for key, info in MAIN_BUTTONS.items():
        # Admin Panel is shown in this manager too, but only affects admin reply-keyboard visibility.
        state = main_button_enabled(key)
        icon = "🟢 ON" if state else "🔴 OFF"
        rows.append([InlineKeyboardButton(f"{info['title']} — {icon}", callback_data=f"mainbtn_toggle|{key}", style=KBS.SUCCESS if state else KBS.DANGER)])
    rows.append([InlineKeyboardButton("⬅️ BACK TO ADMIN", callback_data="admin_back", style=KBS.PRIMARY)])
    return InlineKeyboardMarkup(rows)

async def main_button_control(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    text = (
        "━━━━━━━━━━━━━━━━━━\n"
        "🎛️ <b>MAIN BUTTON CONTROL</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "Tap a button to switch it <b>ON / OFF</b>.\n"
        "OFF buttons are hidden from the main menu.\n"
        "━━━━━━━━━━━━━━━━━━"
    )
    if isinstance(update, CallbackQuery):
        await edit_or_send(update, text, reply_markup=main_button_control_keyboard(), parse_mode="HTML", context=context, auto_delete=False)
    else:
        await send_clean_message(update, context, user_id, text, reply_markup=main_button_control_keyboard(), parse_mode="HTML", auto_delete=False)

async def main_button_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if key not in MAIN_BUTTONS:
        ...
        return
    new_state = not main_button_enabled(key)
    set_main_button_enabled(key, new_state)
    await q.answer("ON" if new_state else "OFF")
    await main_button_control(q, context, uid)
    # Refresh the admin's own reply keyboard immediately. Other users receive the new
    # keyboard the next time the main menu is rendered.
    try:
        await context.bot.send_message(uid, "🔄 Main menu updated.", reply_markup=bottom_menu_keyboard(uid))
    except Exception:
        pass

def bottom_menu_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    rows = []
    if main_button_enabled("get_number"):
        rows.append([KeyboardButton(BTN_GET_NUMBER, style=KBS.SUCCESS, icon_custom_emoji_id=safe_icon(CUSTOM_EMOJIS.get("GET_NUMBER", "")))])
    pair = []
    if main_button_enabled("find_otp"):
        pair.append(KeyboardButton(BTN_FIND_OTP, style=KBS.PRIMARY, icon_custom_emoji_id=safe_icon(_EMOJI_FIND_OTP)))
    if main_button_enabled("live_traffic"):
        pair.append(KeyboardButton(BTN_LIVE, style=KBS.PRIMARY, icon_custom_emoji_id=safe_icon(_EMOJI_LIVE)))
    if pair:
        rows.append(pair)
    pair = []
    if main_button_enabled("balance"):
        pair.append(KeyboardButton(BTN_BALANCE, style=KBS.SUCCESS, icon_custom_emoji_id=safe_icon("5312123810638483121")))
    if main_button_enabled("refer"):
        pair.append(KeyboardButton(BTN_REFER, style=KBS.SUCCESS, icon_custom_emoji_id=safe_icon(_EMOJI_REFER)))
    if pair:
        rows.append(pair)
    if main_button_enabled("contact"):
        rows.append([KeyboardButton(BTN_CONTACT, style=KBS.PRIMARY, icon_custom_emoji_id=safe_icon(_EMOJI_CONTACT))])
    if main_button_enabled("proxy"):
        rows.append([KeyboardButton(BTN_PROXY, style=KBS.SUCCESS, icon_custom_emoji_id=safe_icon(CUSTOM_EMOJIS.get("PROXY", "")))])
    if is_admin(user_id) and main_button_enabled("admin"):
...
def admin_panel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("🔥 PROXY MANAGER", callback_data="admin_proxy_manager", style=KBS.SUCCESS,
                                  icon_custom_emoji_id=safe_icon(CUSTOM_EMOJIS.get("PROXY", ""))),
        ],
        [
            InlineKeyboardButton("🎛️ MAIN BUTTONS ON/OFF", callback_data="admin_main_button_control", style=KBS.PRIMARY),
        ],
        [
...
# ================= PROXY / LINK HUB =================
def get_proxy_links():
    links = get_setting("proxy_links", [])
    if not isinstance(links, list): return []
    return [{"id": str(x.get("id")), "name": str(x.get("name")), "url": str(x.get("url"))}
            for x in links if isinstance(x, dict) and x.get("id") and x.get("name") and x.get("url")]

def save_proxy_links(links):
    update_setting("proxy_links", links)

def proxy_admin_keyboard():
    rows=[]
    for item in get_proxy_links():
        rows.append([InlineKeyboardButton(f"🔗 {item['name'][:35]}", callback_data=f"proxy_admin_edit|{item['id']}", style=KBS.PRIMARY)])
        rows.append([InlineKeyboardButton("✏️ EDIT", callback_data=f"proxy_admin_edit|{item['id']}", style=KBS.SUCCESS), InlineKeyboardButton("🗑 DELETE", callback_data=f"proxy_admin_delete|{item['id']}", style=KBS.DANGER)])
    if not rows: rows.append([InlineKeyboardButton("No buttons added yet", callback_data="noop", style=KBS.PRIMARY)])
    rows.append([InlineKeyboardButton("➕ ADD NEW BUTTON", callback_data="proxy_admin_add", style=KBS.SUCCESS)])
    rows.append([InlineKeyboardButton("⬅️ BACK TO ADMIN", callback_data="admin_back", style=KBS.PRIMARY)])
    return InlineKeyboardMarkup(rows)

def proxy_user_keyboard():
    rows=[[InlineKeyboardButton(f"🔥 {x['name'][:50]}", url=x["url"], style=KBS.SUCCESS)] for x in get_proxy_links()]
    rows.append([InlineKeyboardButton("⬅️ BACK", callback_data="back_to_menu", style=KBS.PRIMARY)])
    return InlineKeyboardMarkup(rows)

async def show_proxy_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    links=get_proxy_links()
    text="━━━━━━━━━━━━━━━━━━\n🔥 <b>PROXY / VPN HUB</b>\n━━━━━━━━━━━━━━━━━━\n" + ("Select a service below to open it:" if links else "No Proxy/VPN services are available right now.") + "\n━━━━━━━━━━━━━━━━━━"
    if isinstance(update, CallbackQuery): await edit_or_send(update,text,reply_markup=proxy_user_keyboard(),parse_mode="HTML",context=context,auto_delete=False)
    else: await send_clean_message(update,context,update.effective_user.id,text,reply_markup=proxy_user_keyboard(),parse_mode="HTML",auto_delete=False)

async def proxy_admin_manager(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    if not is_admin(user_id):
        if isinstance(update, CallbackQuery): await update.answer("Admin only!",show_alert=True)
        return
    text="━━━━━━━━━━━━━━━━━━\n🔥 <b>PROXY / VPN MANAGER</b>\n━━━━━━━━━━━━━━━━━━\nCreate multiple buttons. Each button has its own name + URL.\n━━━━━━━━━━━━━━━━━━"
    if isinstance(update, CallbackQuery): await edit_or_send(update,text,reply_markup=proxy_admin_keyboard(),parse_mode="HTML",context=context,auto_delete=False)
    else: await send_clean_message(update,context,user_id,text,reply_markup=proxy_admin_keyboard(),parse_mode="HTML",auto_delete=False)

async def proxy_admin_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; uid=q.from_user.id
    if not is_admin(uid): await q.answer("Admin only!",show_alert=True); return
    await q.answer(); admin_temp_data[uid]={"proxy_action":"add"}; admin_panel_state[uid]="proxy_add_button"
    await edit_or_send(q,"➕ <b>ADD PROXY/VPN BUTTON</b>\n\nSend:\n<code>Button Name | https://example.com</code>\n\n/cancel to stop.",reply_markup=admin_cancel_keyboard(),parse_mode="HTML",context=context,auto_delete=False)

async def proxy_admin_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; uid=q.from_user.id
    if not is_admin(uid): await q.answer("Admin only!",show_alert=True); return
    pid=q.data.split("|",1)[1]; item=next((x for x in get_proxy_links() if x["id"]==pid),None)
    if not item: await q.answer("Button not found!",show_alert=True); return
    await q.answer(); admin_temp_data[uid]={"proxy_action":"edit","proxy_id":pid}; admin_panel_state[uid]="proxy_add_button"
    await edit_or_send(q,f"✏️ <b>EDIT BUTTON</b>\n\nCurrent: <b>{html.escape(item['name'])}</b>\n<code>{html.escape(item['url'])}</code>\n\nSend:\n<code>Button Name | https://example.com</code>",reply_markup=admin_cancel_keyboard(),parse_mode="HTML",context=context,auto_delete=False)

async def proxy_admin_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; uid=q.from_user.id
    if not is_admin(uid): await q.answer("Admin only!",show_alert=True); return
    pid=q.data.split("|",1)[1]; item=next((x for x in get_proxy_links() if x["id"]==pid),None)
    if not item: await q.answer("Button not found!",show_alert=True); return
    await q.answer(); kb=InlineKeyboardMarkup([[InlineKeyboardButton("✅ YES, DELETE",callback_data=f"proxy_admin_delete_yes|{pid}",style=KBS.DANGER)],[InlineKeyboardButton("⬅️ CANCEL",callback_data="proxy_admin_manager",style=KBS.PRIMARY)]])
    await edit_or_send(q,f"🗑 <b>Delete button?</b>\n\n{html.escape(item['name'])}",reply_markup=kb,parse_mode="HTML",context=context,auto_delete=False)

async def proxy_admin_delete_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; uid=q.from_user.id
    if not is_admin(uid): await q.answer("Admin only!",show_alert=True); return
    pid=q.data.split("|",1)[1]; save_proxy_links([x for x in get_proxy_links() if x["id"]!=pid]); await q.answer("Deleted"); await proxy_admin_manager(q,context,uid)

async def proxy_handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid=update.effective_user.id
    if admin_panel_state.get(uid)!="proxy_add_button": return False
    text=update.message.text.strip()
    if text.lower()=="/cancel":
        admin_temp_data.pop(uid,None); admin_panel_state[uid]="main"; await update.message.reply_text("❌ Cancelled.",reply_markup=admin_panel_keyboard()); return True
    if "|" not in text:
        await update.message.reply_text("❌ Format: <code>Button Name | https://example.com</code>",parse_mode="HTML"); return True
    name,url=[x.strip() for x in text.split("|",1)]
    if not name or not url.startswith(("http://","https://")):
        await update.message.reply_text("❌ Use a name and a valid http/https URL."); return True
    data=admin_temp_data.get(uid,{ }); links=get_proxy_links()
    if data.get("proxy_action")=="edit":
        for x in links:
            if x["id"]==data.get("proxy_id"): x.update({"name":name,"url":url}); break
        else: await update.message.reply_text("❌ Button not found."); return True
        msg="✅ Proxy/VPN button updated."
    else:
        links.append({"id":str(int(time.time()*1000)),"name":name,"url":url}); msg="✅ Proxy/VPN button added."
    save_proxy_links(links); admin_temp_data.pop(uid,None); admin_panel_state[uid]="main"
    await update.message.reply_text(msg); await proxy_admin_manager(update,context,uid); return True
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "stats":
        ...
        await admin_otp_group(update, context)
    elif action == "proxy_manager":
        await proxy_admin_manager(update, context, user_id)
    elif action == "main_button_control":
        await main_button_control(update, context, user_id)
    elif action == "exit":
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await handle_admin_text(update, context):
        return
    if await proxy_handle_text(update, context):
        return
    ...
    if await ban_check(update, context):
        return
    text = update.message.text.strip()
    if text == BTN_GET_NUMBER and main_button_enabled("get_number"):
        await send_get_number_panel(update, context)
    elif text == BTN_BALANCE and main_button_enabled("balance"):
        await send_balance_panel(update, context)
    elif text == BTN_INVITE and main_button_enabled("refer"):
        await show_invite(update, context)
    elif text == BTN_SUPPORT and main_button_enabled("contact"):
        await send_support_panel(update, context)
    elif text == BTN_ADMIN and is_admin(update.effective_user.id) and main_button_enabled("admin"):
        await send_admin_panel_msg(update, context)
    elif text == BTN_FIND_OTP and main_button_enabled("find_otp"):
        await send_find_otp_panel(update, context)
    elif text == BTN_LIVE and main_button_enabled("live_traffic"):
        await send_live_traffic_panel(update, context)
    elif text == BTN_REFER and main_button_enabled("refer"):
        await show_invite(update, context)
    elif text == BTN_CONTACT and main_button_enabled("contact"):
        await send_support_panel(update, context)
    elif text == BTN_PROXY and main_button_enabled("proxy"):
        await show_proxy_panel(update, context)
    elif text == BTN_PROXY:
        await show_proxy_panel(update, context)
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(show_proxy_panel, pattern="^proxy_panel$"))
    application.add_handler(CallbackQueryHandler(proxy_admin_add, pattern="^proxy_admin_add$"))
    application.add_handler(CallbackQueryHandler(proxy_admin_edit, pattern=r"^proxy_admin_edit\|"))
    application.add_handler(CallbackQueryHandler(proxy_admin_delete, pattern=r"^proxy_admin_delete\|"))
    application.add_handler(CallbackQueryHandler(proxy_admin_delete_yes, pattern=r"^proxy_admin_delete_yes\|"))
    application.add_handler(CallbackQueryHandler(proxy_admin_manager, pattern="^proxy_admin_manager$"))
    application.add_handler(CallbackQueryHandler(main_button_control, pattern="^admin_main_button_control$"))
    application.add_handler(CallbackQueryHandler(main_button_toggle, pattern=r"^mainbtn_toggle\|"))
