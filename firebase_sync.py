...
def restore_from_firestore(conn, lock):
    ...
    try:
        ...
        with lock:
            ...
            local_data_rows = sum(counts.values())

        if local_data_rows > 0:
            print(f"ℹ️  Firebase restore skipped: SQLite already contains {local_data_rows} persisted rows.")
            return
...
def get_sync_status():
    """Return non-sensitive Firebase persistence status for diagnostics."""
    return {"enabled": is_enabled(), "last_sync_at": _last_sync_at, "last_sync_error": _last_sync_error, "last_restore_at": _last_restore_at, "worker_alive": bool(_worker and _worker.is_alive())}
