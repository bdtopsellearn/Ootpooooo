...
def bottom_menu_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    rows = [
        [KeyboardButton(BTN_GET_NUMBER, style=KBS.SUCCESS,
                        icon_custom_emoji_id=safe_icon(CUSTOM_EMOJIS.get("GET_NUMBER", "")))],
        [
            KeyboardButton(BTN_FIND_OTP, style=KBS.PRIMARY,
                           icon_custom_emoji_id=safe_icon(_EMOJI_FIND_OTP)),
            KeyboardButton(BTN_LIVE, style=KBS.PRIMARY,
                           icon_custom_emoji_id=safe_icon(_EMOJI_LIVE)),
        ],
        [
            KeyboardButton(BTN_BALANCE, style=KBS.SUCCESS,
                           icon_custom_emoji_id=safe_icon("5312123810638483121")),
            KeyboardButton(BTN_REFER, style=KBS.SUCCESS,
                           icon_custom_emoji_id=safe_icon(_EMOJI_REFER)),
        ],
        [KeyboardButton(BTN_CONTACT, style=KBS.PRIMARY,
                        icon_custom_emoji_id=safe_icon(_EMOJI_CONTACT))],
    ]
    if is_admin(user_id):
        rows.append([KeyboardButton(BTN_ADMIN, style=KBS.DANGER,
                                    icon_custom_emoji_id=safe_icon(CUSTOM_EMOJIS.get("ADMIN", "")))])
