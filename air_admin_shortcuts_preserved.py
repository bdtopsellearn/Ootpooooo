...
from firebase_sync import (
    ...
    mark_dirty      as firebase_mark_dirty,
    sync_table_now  as firebase_sync_table,
    force_sync_now  as firebase_force_sync,
    restore_from_firestore  as firebase_restore_if_needed,
    start_sync_worker       as firebase_start_worker,
    sync_to_firestore       as firebase_sync_now,
)
...
# ================= ISOLATED DEMO / FAKE OTP SYSTEM =================
# This simulator is intentionally independent of real OTP/API polling.
# It never reads OTPs, numbers, API panels, or the real OTP group list.
FAKE_OTP_DEFAULT_INTERVAL = 10
FAKE_OTP_MIN_INTERVAL = 1
FAKE_OTP_MAX_INTERVAL = 1440
FAKE_OTP_LAST_SENT_KEY = "fake_otp_last_sent_at"

def fake_otp_status_text():
    enabled = str(get_bot_setting("fake_otp_enabled", "0")).lower() in ("1", "true", "yes", "on")
    country = get_bot_setting("fake_otp_country", "Bangladesh")
    service = get_bot_setting("fake_otp_service", "Demo Service")
    interval = get_bot_setting("fake_otp_interval", str(FAKE_OTP_DEFAULT_INTERVAL))
    group_id = get_bot_setting("fake_otp_group_id", "")
    group = group_id if group_id else "NOT SET"
    return (
        "🧪 <b>DEMO / FAKE OTP SYSTEM</b>\n\n"
        f"Status: {'🟢 ON' if enabled else '🔴 OFF'}\n"
        f"🌍 Country: <b>{html.escape(str(country))}</b>\n"
        f"📱 Service: <b>{html.escape(str(service))}</b>\n"
        f"⏱ Interval: <b>{html.escape(str(interval))} min</b>\n"
        f"👥 Demo Group: <b>{html.escape(str(group))}</b>\n\n"
        "⚠️ Every generated message is explicitly marked DEMO / FAKE OTP."
    )

def fake_otp_keyboard():
    enabled = str(get_bot_setting("fake_otp_enabled", "0")).lower() in ("1", "true", "yes", "on")
    toggle = "⏹ STOP DEMO" if enabled else "▶️ START DEMO"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(toggle, callback_data="fakeotp_toggle", style=KBS.SUCCESS if not enabled else KBS.DANGER)],
        [InlineKeyboardButton("🌍 SET COUNTRY", callback_data="fakeotp_country", style=KBS.PRIMARY),
         InlineKeyboardButton("📱 SET SERVICE", callback_data="fakeotp_service", style=KBS.PRIMARY)],
        [InlineKeyboardButton("⏱ SET INTERVAL", callback_data="fakeotp_interval", style=KBS.PRIMARY),
         InlineKeyboardButton("👥 SET DEMO GROUP", callback_data="fakeotp_group", style=KBS.PRIMARY)],
        [InlineKeyboardButton("🧪 SEND TEST NOW", callback_data="fakeotp_test", style=KBS.SUCCESS)],
        [InlineKeyboardButton("🔙 BACK", callback_data="admin_back", style=KBS.DANGER)],
    ])

def fake_otp_generate_message():
    # Synthetic values only: never derived from stock, users, API panels, or OTP data.
    country = str(get_bot_setting("fake_otp_country", "Bangladesh"))
    service = str(get_bot_setting("fake_otp_service", "Demo Service"))
    otp = f"{random.randint(0, 999999):06d}"
    number = "+880 17XX XXX XXX" if country.lower() == "bangladesh" else "XXX XXX XXXX"
    return (
        "🧪 <b>DEMO / FAKE OTP</b>\n\n"
        f"🌍 Country: <b>{html.escape(country)}</b>\n"
        f"📱 Service: <b>{html.escape(service)}</b>\n"
        f"📞 Demo Number: <code>{html.escape(number)}</code>\n"
        f"🔐 Demo OTP: <code>{otp}</code>\n\n"
        "⚠️ <b>DEMO ONLY</b> — this code is randomly generated and is not a real verification code."
    )

async def fake_otp_send_once(bot):
    group_id = str(get_bot_setting("fake_otp_group_id", "")).strip()
    if not group_id:
        return False, "Demo Group is not set."
    try:
        await bot.send_message(chat_id=int(group_id), text=fake_otp_generate_message(), parse_mode="HTML")
        set_bot_setting(FAKE_OTP_LAST_SENT_KEY, str(time.time()))
        return True, "Demo OTP sent."
    except Exception as exc:
        return False, f"Demo send failed: {exc}"

async def fake_otp_job(context: ContextTypes.DEFAULT_TYPE):
    try:
        enabled = str(get_bot_setting("fake_otp_enabled", "0")).lower() in ("1", "true", "yes", "on")
        if not enabled:
            return
        try:
            interval = max(FAKE_OTP_MIN_INTERVAL, min(FAKE_OTP_MAX_INTERVAL, int(float(get_bot_setting("fake_otp_interval", FAKE_OTP_DEFAULT_INTERVAL)))))
        except (TypeError, ValueError):
            interval = FAKE_OTP_DEFAULT_INTERVAL
        last = float(get_bot_setting(FAKE_OTP_LAST_SENT_KEY, "0") or 0)
        if time.time() - last < interval * 60:
            return
        await fake_otp_send_once(context.bot)
    except Exception as exc:
        print(f"⚠️ Demo OTP job error: {exc}")

async def admin_fake_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        ...
        return
    admin_panel_state[user_id] = "fake_otp"
    text = fake_otp_status_text()
    if query:
        await edit_or_send(query, text, reply_markup=fake_otp_keyboard(), parse_mode="HTML", context=context, auto_delete=False)
    else:
        await send_clean_message(update, context, user_id, text, reply_markup=fake_otp_keyboard(), parse_mode="HTML", auto_delete=False)

async def fake_otp_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    ...
    if not is_admin(user_id):
        ...
        return
    data = query.data
    if data == "fakeotp_toggle":
        enabled = str(get_bot_setting("fake_otp_enabled", "0")).lower() in ("1", "true", "yes", "on")
        update_setting("fake_otp_enabled", "0" if enabled else "1")
        if enabled:
            update_setting(FAKE_OTP_LAST_SENT_KEY, "0")
        await query.answer("Demo system stopped." if enabled else "Demo system started.")
    elif data == "fakeotp_country":
        admin_panel_state[user_id] = "fakeotp_wait_country"
        await query.answer()
        await query.message.reply_text("🌍 Send the demo country name (example: Bangladesh):", reply_markup=admin_cancel_keyboard())
        return
    elif data == "fakeotp_service":
        admin_panel_state[user_id] = "fakeotp_wait_service"
        await query.answer()
        await query.message.reply_text("📱 Send the demo service name (example: TikTok):", reply_markup=admin_cancel_keyboard())
        return
    elif data == "fakeotp_interval":
        admin_panel_state[user_id] = "fakeotp_wait_interval"
        await query.answer()
        await query.message.reply_text("⏱ Send interval in minutes (1–1440):", reply_markup=admin_cancel_keyboard())
        return
    elif data == "fakeotp_group":
        admin_panel_state[user_id] = "fakeotp_wait_group"
        await query.answer()
        kb = selection_reply_keyboard("👥 SELECT DEMO GROUP", 1005, is_channel=False)
        await query.message.reply_text("👇 Select a separate <b>DEMO GROUP</b>.\nDo not use the real OTP group.", reply_markup=kb, parse_mode="HTML")
        await query.message.delete()
        return
    elif data == "fakeotp_test":
        ok, msg = await fake_otp_send_once(context.bot)
        await query.answer(msg, show_alert=True)
    await admin_fake_otp(update, context)
...
def load_premium_apps():
    ...
    try:
        ...
        with open(path, 'w', encoding='utf-8') as f:
            f.write("# PREMUAM_APPS.txt\n# Format: service_name|fallback_emoji|premium_emoji_id\n")
            for name, info in default.items():
                f.write(f"{name}|{info['emoji']}|{info['id']}\n")
        mapping = default
    except Exception as e:
        print(f"Error loading PREMUAM_APPS.txt: {e}")
        mapping = {}
    return mapping
...
# Enable WAL and performance settings
...
c.execute("CREATE INDEX IF NOT EXISTS idx_numbers_user_id ON numbers(user_id)")
c.execute("CREATE INDEX IF NOT EXISTS idx_numbers_status ON numbers(status)")
c.execute("CREATE INDEX IF NOT EXISTS idx_otps_user_id ON otps(user_id)")
c.execute("CREATE INDEX IF NOT EXISTS idx_otps_number ON otps(number)")
c.execute("CREATE INDEX IF NOT EXISTS idx_api_logs_api_id ON api_logs(api_id)")
c.execute("CREATE INDEX IF NOT EXISTS idx_cdr_logs_panel_id ON cdr_logs(panel_id)")
...
# ================= LOCAL DURABILITY / RECOVERY =================
LOCAL_BACKUP_DIR = os.path.join(DB_DIR, "recovery")
LOCAL_BACKUP_PATH = os.path.join(LOCAL_BACKUP_DIR, "latest.db")
os.makedirs(LOCAL_BACKUP_DIR, exist_ok=True)

def create_local_db_backup():
    ...
    try:
        ...
        print(f"⚠️ Local DB backup failed: {exc}")
        try:
            if os.path.exists(tmp_path): os.remove(tmp_path)
        except Exception: pass
        return False

def database_integrity_status():
    try:
        with db_lock:
...
# SQLite is always the live DB. Firebase provides cloud backup/restore only.
firebase_init_ok = init_firebase(conn, db_lock)
if firebase_init_ok:
    firebase_restore_if_needed(conn, db_lock)   # restore if fresh filesystem
    firebase_start_worker(conn, db_lock, interval=30)  # background sync every 30s
...
def extract_otp_from_message(message: str) -> str | None:
    ...
    if not all_matches:
        # Try to find any sequence of digits of length 4-8 as a last resort
        fallback = re.search(r'\b(\d{4,8})\b', message)
        if fallback:
...
def db_exec(query, params=()):
    """Execute a write durably, commit to SQLite, then trigger cloud sync."""
    with db_lock:
...
def db_exec_sync(query, params=(), tables=None):
    """
    Execute a write query AND immediately sync the given tables to Firebase.
    Use for important admin panel changes (settings, services, numbers, etc.)
    so data is safe even if the bot crashes right after.
    tables: list of table names to sync, or None to sync everything.
    """
    db_exec(query, params)
    if firebase_enabled():
        if tables:
            for tbl in tables:
                firebase_sync_table(tbl)
        else:
            firebase_force_sync()
...
def set_bot_setting(key, value):
    ...
    # Immediately sync bot_settings to Firebase after every admin panel change
    if firebase_enabled():
        firebase_sync_table("bot_settings")
...
BTN_FIND_OTP    = "FIND OTP"
BTN_LIVE        = "LIVE TRAFFIC"
BTN_REFER       = "REFER AND EARN"
BTN_CONTACT     = "CONTACT"

# Emoji IDs for new buttons
_EMOJI_FIND_OTP = "5300225321785951963"
_EMOJI_LIVE     = "5338747584920512794"
_EMOJI_REFER    = "5384394344859974865"
_EMOJI_CONTACT  = "6264853036993090338"

# Main reply keyboard - layout matches screenshot:
#   Row 1: [ GET NUMBER ]          full-width, green
#   Row 2: [ FIND OTP ] [ LIVE TRAFFIC ]       blue
#   Row 3: [ BALANCE ] [ REFER AND EARN ]      green
#   Row 4: [ CONTACT ]             full-width, blue
#   Row 5: [ Admin Panel ]         full-width, red (admins only)
def bottom_menu_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    """Professional user dashboard keyboard with quick-access shortcuts."""
    rows = [
        [KeyboardButton("⚡ GET NUMBER", style=KBS.SUCCESS, icon_custom_emoji_id=safe_icon(CUSTOM_EMOJIS.get("GET_NUMBER", "")))],
        [KeyboardButton("📊 MY ACTIVITY", style=KBS.PRIMARY), KeyboardButton("💰 BALANCE", style=KBS.SUCCESS, icon_custom_emoji_id=safe_icon("5312123810638483121"))],
        [KeyboardButton("👤 MY ACCOUNT", style=KBS.PRIMARY), KeyboardButton("🤝 REFER & EARN", style=KBS.SUCCESS, icon_custom_emoji_id=safe_icon(_EMOJI_REFER))],
        [KeyboardButton("🔎 FIND OTP", style=KBS.PRIMARY, icon_custom_emoji_id=safe_icon(_EMOJI_FIND_OTP)), KeyboardButton("📡 LIVE TRAFFIC", style=KBS.PRIMARY, icon_custom_emoji_id=safe_icon(_EMOJI_LIVE))],
        [KeyboardButton("🛠 SETTINGS", style=KBS.PRIMARY), KeyboardButton("❓ HELP CENTER", style=KBS.PRIMARY)],
        [KeyboardButton("📞 SUPPORT", style=KBS.PRIMARY, icon_custom_emoji_id=safe_icon(_EMOJI_CONTACT)), KeyboardButton("🔄 REFRESH", style=KBS.PRIMARY)],
    ]
    if is_admin(user_id):
        rows.append([KeyboardButton("🛡 ADMIN CONTROL", style=KBS.DANGER, icon_custom_emoji_id=safe_icon(CUSTOM_EMOJIS.get("ADMIN", "")))])
...
def admin_panel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("⟦𝐗𝐞𝐃𝐈𝐍𝐆𝐉𝐀𝐌𝐄𝐬⟧", callback_data="admin_air_control", style=KBS.DANGER,
                                 icon_custom_emoji_id=safe_icon(AIR_CONTROL_EMOJI)),
        ...
        [
            InlineKeyboardButton("🧪 DEMO OTP", callback_data="fakeotp_menu", style=KBS.PRIMARY),
    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                ...
                style=KBS.SUCCESS,
                icon_custom_emoji_id=safe_icon("5420394784619831222")
            ),
            InlineKeyboardButton(
...
def get_otp_group_keyboard():
    ...
    # Number Channel
    nc_link = get_setting("number_channel_link", "")
    nc_display = nc_link if nc_link else "Not Set"
    if len(nc_display) > 25:
        nc_display = nc_display[:25] + "..."
    rows.append([
        InlineKeyboardButton(
            f"Number Channel: {nc_display}",
            callback_data="edit_number_channel",
            style=KBS.PRIMARY,
            icon_custom_emoji_id=safe_icon("5429353834881261942")
        )
    ])

    # Method Link (TSB X METHOD)
    ml_link = get_setting("method_link", "")
    ml_display = ml_link if ml_link else "Not Set"
    if len(ml_display) > 25:
        ml_display = ml_display[:25] + "..."
    rows.append([
        InlineKeyboardButton(
            f"Method Link: {ml_display}",
            callback_data="edit_method_link",
            style=KBS.PRIMARY,
            icon_custom_emoji_id=safe_icon("5429353834881261942")
        )
    ])
    ...
    # Inbox Link
    ib_link = get_setting("inbox_link", "")
    ib_display = ib_link if ib_link else "Not Set"
    if len(ib_display) > 25:
        ib_display = ib_display[:25] + "..."
    rows.append([
        InlineKeyboardButton(
            f"Inbox Link: {ib_display}",
            callback_data="edit_inbox_link",
            style=KBS.PRIMARY,
            icon_custom_emoji_id=safe_icon("5429353834881261942")
        )
    ])
    ...
    otp_forward_groups = get_otp_group_ids()
    ...
    rows.append([
        InlineKeyboardButton(
            "Set Forward Group",
            callback_data="otp_select_group",
            style=KBS.SUCCESS,
            icon_custom_emoji_id=safe_icon("5429501315468270290")
        )
    ])
...
def start_welcome_html():
    ...
    return ("╔════════════════════════════════╗\n"
            f"║   {wave} <b>JAMES PRO</b> • <b>WELCOME</b>   ║\n"
            "╚════════════════════════════════╝\n\n"
            "⚡ <b>FAST • CLEAN • SECURE</b>\n"
            f"{inbox} <b>Your personal dashboard is ready.</b>\n"
            f"{money} <b>Manage your account, activity and services from the quick menu below.</b>\n\n"
            "🔐 <i>Account data uses local database + cloud recovery protection.</i>")
...
async def handle_withdraw_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    request_id = db_fetch_one("SELECT last_insert_rowid()")[0]
    user_states[user_id] = {"state": f"waiting_withdraw_number_{method}_{amount}_{request_id}", "msg_id": target_msg_id}
...
async def admin_withdraw_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not req:
        ...
        return
    req_id, req_user_id, amount, method, account_number, status = req
    if status != 'pending':
        await query.answer(f"This request has already been {status}.", show_alert=True)
        return
    ...
    # Sync users and withdraw_requests after financial operation
    if firebase_enabled():
        firebase_sync_table("users")
        firebase_sync_table("withdraw_requests")
...
# New UI/control-center layer. Existing production handlers remain untouched.
# DEMO OTP remains isolated from real OTP/API forwarding.

_PRO_MODULES = {
    "pro_dashboard":"DASHBOARD", "pro_users":"USER MANAGER",
    "pro_stock":"STOCK CONTROL", "pro_services":"SERVICE MANAGER",
    "pro_countries":"COUNTRY MANAGER", "pro_broadcast":"BROADCAST CENTER",
    "pro_database":"DATABASE", "pro_firebase":"FIREBASE SYNC",
    "pro_health":"BOT HEALTH", "pro_uptime":"UPTIME",
    "pro_logs":"SYSTEM LOGS", "pro_errors":"ERROR CENTER",
    "pro_security":"SECURITY", "pro_admins":"ADMIN MANAGER",
    "pro_maintenance":"MAINTENANCE", "pro_backup":"BACKUP CENTER",
    "pro_restore":"RESTORE CENTER", "pro_settings":"BOT SETTINGS",
    "pro_theme":"THEME CONTROL", "pro_language":"LANGUAGE",
    "pro_welcome":"WELCOME", "pro_notice":"NOTICE CENTER",
    "pro_support":"SUPPORT", "pro_limits":"RATE LIMIT",
    "pro_scheduler":"SCHEDULER", "pro_autodelete":"AUTO DELETE",
    "pro_forcejoin":"FORCE JOIN", "pro_groups":"GROUP CONTROL",
    "pro_analytics":"ANALYTICS", "pro_activity":"ACTIVITY",
    "pro_export":"EXPORT CENTER", "pro_import":"IMPORT CENTER",
    "pro_demo":"DEMO OTP", "pro_system":"SYSTEM INFO",
    "pro_about":"ABOUT / VERSION",
}

def premium_admin_keyboard():
    rows = [
        [InlineKeyboardButton("📊 𝐃𝐀𝐒𝐇𝐁𝐎𝐀𝐑𝐃", callback_data="pro_dashboard", style=KBS.PRIMARY),
         InlineKeyboardButton("👤 𝐔𝐒𝐄𝐑𝐒", callback_data="pro_users", style=KBS.PRIMARY)],
        [InlineKeyboardButton("📦 𝐒𝐓𝐎𝐂𝐊", callback_data="pro_stock", style=KBS.SUCCESS),
         InlineKeyboardButton("🧩 𝐒𝐄𝐑𝐕𝐈𝐂𝐄𝐒", callback_data="pro_services", style=KBS.PRIMARY)],
        [InlineKeyboardButton("🌍 𝐂𝐎𝐔𝐍𝐓𝐑𝐈𝐄𝐒", callback_data="pro_countries", style=KBS.PRIMARY),
         InlineKeyboardButton("📣 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓", callback_data="pro_broadcast", style=KBS.PRIMARY)],
        [InlineKeyboardButton("☁️ 𝐅𝐈𝐑𝐄𝐁𝐀𝐒𝐄", callback_data="pro_firebase", style=KBS.SUCCESS),
         InlineKeyboardButton("🗄️ 𝐃𝐀𝐓𝐀𝐁𝐀𝐒𝐄", callback_data="pro_database", style=KBS.SUCCESS)],
        [InlineKeyboardButton("🩺 𝐇𝐄𝐀𝐋𝐓𝐇", callback_data="pro_health", style=KBS.SUCCESS),
         InlineKeyboardButton("⏱️ 𝐔𝐏𝐓𝐈𝐌𝐄", callback_data="pro_uptime", style=KBS.SUCCESS)],
        [InlineKeyboardButton("🛡️ 𝐒𝐄𝐂𝐔𝐑𝐈𝐓𝐘", callback_data="pro_security", style=KBS.DANGER),
         InlineKeyboardButton("👑 𝐀𝐃𝐌𝐈𝐍𝐒", callback_data="pro_admins", style=KBS.DANGER)],
        [InlineKeyboardButton("🔧 𝐌𝐀𝐈𝐍𝐓𝐄𝐍𝐀𝐍𝐂𝐄", callback_data="pro_maintenance", style=KBS.DANGER),
         InlineKeyboardButton("💾 𝐁𝐀𝐂𝐊𝐔𝐏", callback_data="pro_backup", style=KBS.SUCCESS)],
        [InlineKeyboardButton("⚙️ 𝐒𝐄𝐓𝐓𝐈𝐍𝐆𝐒", callback_data="pro_settings", style=KBS.PRIMARY),
         InlineKeyboardButton("🎨 𝐓𝐇𝐄𝐌𝐄", callback_data="pro_theme", style=KBS.PRIMARY)],
        [InlineKeyboardButton("📢 𝐍𝐎𝐓𝐈𝐂𝐄𝐒", callback_data="pro_notice", style=KBS.PRIMARY),
         InlineKeyboardButton("🎧 𝐒𝐔𝐏𝐏𝐎𝐑𝐓", callback_data="pro_support", style=KBS.PRIMARY)],
        [InlineKeyboardButton("📈 𝐀𝐍𝐀𝐋𝐘𝐓𝐈𝐂𝐒", callback_data="pro_analytics", style=KBS.PRIMARY),
         InlineKeyboardButton("📜 𝐋𝐎𝐆𝐒", callback_data="pro_logs", style=KBS.PRIMARY)],
        [InlineKeyboardButton("🧪 𝐃𝐄𝐌𝐎 𝐎𝐓𝐏", callback_data="fakeotp_menu", style=KBS.PRIMARY),
         InlineKeyboardButton("💻 𝐒𝐘𝐒𝐓𝐄𝐌", callback_data="pro_system", style=KBS.PRIMARY)],
        [InlineKeyboardButton("⚡ 𝐐𝐔𝐈𝐂𝐊 𝐒𝐇𝐎𝐑𝐓𝐂𝐔𝐓𝐒", callback_data="pro_shortcuts", style=KBS.SUCCESS)],
        [InlineKeyboardButton("🔙 𝐁𝐀𝐂𝐊", callback_data="admin_back", style=KBS.PRIMARY)]
    ]
    return InlineKeyboardMarkup(rows)

def premium_quick_shortcuts_keyboard():
    """Quick access layer only; all existing admin systems remain unchanged."""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📊 USER STATS", callback_data="quick_stats", style=KBS.PRIMARY),
         InlineKeyboardButton("👤 USER MANAGER", callback_data="quick_users", style=KBS.PRIMARY)],
        [InlineKeyboardButton("📦 STOCK TOOLS", callback_data="quick_stock", style=KBS.SUCCESS),
         InlineKeyboardButton("🌍 COUNTRY MANAGER", callback_data="quick_countries", style=KBS.PRIMARY)],
        [InlineKeyboardButton("🧩 SERVICE MANAGER", callback_data="quick_services", style=KBS.PRIMARY),
         InlineKeyboardButton("🗄️ DATABASE", callback_data="quick_database", style=KBS.SUCCESS)],
        [InlineKeyboardButton("☁️ FIREBASE", callback_data="quick_firebase", style=KBS.SUCCESS),
         InlineKeyboardButton("🩺 BOT HEALTH", callback_data="quick_health", style=KBS.SUCCESS)],
        [InlineKeyboardButton("📣 BROADCAST", callback_data="quick_broadcast", style=KBS.PRIMARY),
         InlineKeyboardButton("🛡️ FORCE JOIN", callback_data="quick_forcejoin", style=KBS.PRIMARY)],
        [InlineKeyboardButton("🔙 CONTROL CENTER", callback_data="pro_center", style=KBS.PRIMARY)]
    ])

async def premium_quick_shortcuts_callback(update, context):
    ...
    await q.answer()
    action = q.data
    if action == "pro_shortcuts":
        await edit_or_send(q, "⚡ <b>QUICK ADMIN SHORTCUTS</b>\n\nFast access to existing admin systems.\n<b>No legacy system is removed or replaced.</b>",
                           reply_markup=premium_quick_shortcuts_keyboard(), parse_mode="HTML", context=context, auto_delete=False)
    elif action == "quick_stats":
        await show_admin_stats(q, uid, context)
    elif action == "quick_users":
        await _user_manager_wrapper(q, context)
    elif action == "quick_stock":
        await stock_management_menu(q, context, uid)
    elif action == "quick_countries":
        await country_manager_menu(q, uid, context)
    elif action == "quick_services":
        await service_manager_menu(q, uid, context)
    elif action == "quick_database":
        await _database_wrapper(q, context)
    elif action == "quick_firebase":
        await premium_control_center(q, context, uid)
        await edit_or_send(q, "☁️ <b>FIREBASE QUICK ACCESS</b>\n\nUse the Firebase module for sync/status controls.",
                           reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("☁️ OPEN FIREBASE", callback_data="pro_firebase", style=KBS.SUCCESS)], [InlineKeyboardButton("🔙 QUICK SHORTCUTS", callback_data="pro_shortcuts", style=KBS.PRIMARY)]]), parse_mode="HTML", context=context, auto_delete=False)
    elif action == "quick_health":
        await premium_module_callback(type("Obj", (), {"callback_query": q})(), context)
    elif action == "quick_broadcast":
        await request_broadcast(q, uid, context)
    elif action == "quick_forcejoin":
        await admin_force_join(q, context)

def premium_module_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 𝐑𝐄𝐅𝐑𝐄𝐒𝐇", callback_data="pro_refresh", style=KBS.PRIMARY)],
        [InlineKeyboardButton("🔙 𝐂𝐎𝐍𝐓𝐑𝐎𝐋 𝐂𝐄𝐍𝐓𝐄𝐑", callback_data="pro_center", style=KBS.PRIMARY)]
    ])

async def premium_control_center(update, context, user_id):
    if not is_admin(user_id):
        if isinstance(update, CallbackQuery):
    ...
    text = (
        ...
        "⚡ <b>FAST</b> • <b>CLEAN</b> • <b>MODULAR</b> • <b>ISOLATED</b>\n\n"
        "Choose a module:"
    )
    if isinstance(update, CallbackQuery):
        await edit_or_send(update, text, reply_markup=premium_admin_keyboard(),
                           context=context, auto_delete=False)
    else:
        await send_clean_message(update, context, user_id, text,
                                 reply_markup=premium_admin_keyboard(), auto_delete=False)

async def premium_module_callback(update, context):
    q = update.callback_query
    uid = q.from_user.id
    if not is_admin(uid):
        await q.answer("Unauthorized!", show_alert=True)
    ...
    key = q.data
    await q.answer()
    if key in ("pro_center", "pro_refresh"):
        await premium_control_center(q, context, uid)
        return
    label = _PRO_MODULES.get(key, "SYSTEM")
    try:
        u = db_fetch_one("SELECT COUNT(*) FROM users")
        s = db_fetch_one("SELECT COUNT(*) FROM services")
        c = db_fetch_one("SELECT COUNT(*) FROM countries")
        users, services, countries = (u[0] if u else 0), (s[0] if s else 0), (c[0] if c else 0)
    except Exception:
        users = services = countries = 0
    text = (
        f"╔══════════════════════════════╗\n"
        f"║   ✦ 𝐉𝐀𝐌𝐄𝐒 𝐏𝐑𝐎 • {label:<12} ✦   ║\n"
        f"╚══════════════════════════════╝\n\n"
        "🟢 <b>MODULE READY</b>\n\n"
        f"👤 Users: <b>{users}</b>\n"
        f"🧩 Services: <b>{services}</b>\n"
        f"🌍 Countries: <b>{countries}</b>\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "This new control-center layer does not alter the real OTP/API forwarding path.\n"
        "━━━━━━━━━━━━━━━━━━━━"
    )
    await edit_or_send(q, text, reply_markup=premium_module_keyboard(),
                       context=context, auto_delete=False)

# ================= ADMIN PANEL MENU =================
async def admin_panel_menu(update, user_id, context: ContextTypes.DEFAULT_TYPE = None):
    await premium_control_center(update, context, user_id)
...
async def periodic_firebase_sync(context: ContextTypes.DEFAULT_TYPE):
    if firebase_enabled():
        firebase_sync_now()
...
async def handle_chat_shared(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif request_id == 1005 and state == "fakeotp_wait_group":
        try:
            chat = await context.bot.get_chat(chat_id)
            if chat.type not in ["group", "supergroup"]:
                await update.message.reply_text("❌ Demo target must be a group/supergroup.")
                admin_panel_state.pop(user_id, None)
                return
            update_setting("fake_otp_group_id", str(chat.id))
            admin_panel_state[user_id] = "fake_otp"
            await update.message.reply_text(f"✅ Demo Group set to '{chat.title}'.")
            await admin_fake_otp(update, context)
        except Exception as exc:
            admin_panel_state.pop(user_id, None)
            await update.message.reply_text(f"❌ Failed to set demo group: {exc}")
    else:
...
async def handle_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "fakeotp_wait_country":
        country = text.strip()
        if not country or len(country) > 50:
            await update.message.reply_text("❌ Invalid country name.")
            return True
        update_setting("fake_otp_country", country)
        admin_panel_state[user_id] = "fake_otp"
        await update.message.reply_text("✅ Demo country updated.")
        await admin_fake_otp(update, context)
        return True
    if state == "fakeotp_wait_service":
        service = text.strip()
        if not service or len(service) > 50:
            await update.message.reply_text("❌ Invalid service name.")
            return True
        update_setting("fake_otp_service", service)
        admin_panel_state[user_id] = "fake_otp"
        await update.message.reply_text("✅ Demo service updated.")
        await admin_fake_otp(update, context)
        return True
    if state == "fakeotp_wait_interval":
        try:
            interval = int(float(text.strip()))
        except ValueError:
            await update.message.reply_text("❌ Enter a whole number from 1 to 1440 minutes.")
            return True
        if not (FAKE_OTP_MIN_INTERVAL <= interval <= FAKE_OTP_MAX_INTERVAL):
            await update.message.reply_text("❌ Interval must be between 1 and 1440 minutes.")
            return True
        update_setting("fake_otp_interval", str(interval))
        admin_panel_state[user_id] = "fake_otp"
        await update.message.reply_text(f"✅ Demo interval set to {interval} minute(s).")
        await admin_fake_otp(update, context)
        return True
    if state == "fakeotp_wait_group":
        await update.message.reply_text("👇 Please use the group-selection button so the demo group is stored safely.")
        return True
    ...
    # AIR CONTROL text handlers
        ...
        if text.strip().startswith('http'):
            ...
            set_bot_setting('main_channel_link', text.strip())
            await update.message.reply_text("✅ Main channel link updated.")
        else:
        ...
        return True
    elif state == "waiting_number_channel":
        if text.strip().startswith('http') or text.strip().startswith('t.me') or text.strip().startswith('@'):
            update_setting('number_channel_link', text.strip())
            set_bot_setting('number_channel_link', text.strip())
            await update.message.reply_text("✅ Number channel link updated.")
        else:
            await update.message.reply_text("Invalid URL. Must start with http/https or t.me")
        admin_panel_state[user_id] = "main"
        await admin_otp_group(update, context)
        await send_with_main_keyboard(update, context, user_id, "✅ Number channel updated.")
        return True
    elif state == "waiting_method_link":
        if text.strip().startswith('http') or text.strip().startswith('t.me') or text.strip().startswith('@'):
            update_setting('method_link', text.strip())
            set_bot_setting('method_link', text.strip())
            await update.message.reply_text("✅ Method link updated.")
        else:
            await update.message.reply_text("Invalid URL. Must start with http/https or t.me")
        admin_panel_state[user_id] = "main"
        await admin_otp_group(update, context)
        await send_with_main_keyboard(update, context, user_id, "✅ Method link updated.")
        return True
    elif state == "waiting_inbox_link":
        if text.strip().startswith('http') or text.strip().startswith('t.me') or text.strip().startswith('@'):
            update_setting('inbox_link', text.strip())
            set_bot_setting('inbox_link', text.strip())
            await update.message.reply_text("✅ Inbox link updated.")
        else:
            await update.message.reply_text("Invalid URL. Must start with http/https or t.me")
        admin_panel_state[user_id] = "main"
        await admin_otp_group(update, context)
        await send_with_main_keyboard(update, context, user_id, "✅ Inbox link updated.")
        return True
    return False
...
async def edit_main_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await edit_or_send(query, "🔗 Enter the <b>Main Channel</b> URL (http://... or t.me/...):",
                       reply_markup=admin_cancel_keyboard(), context=context, auto_delete=False)

async def edit_number_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    if not is_admin(user_id):
        await query.answer("Unauthorized!", show_alert=True)
        return
    await query.answer()
    admin_panel_state[user_id] = "waiting_number_channel"
    await edit_or_send(query, "📞 Enter the <b>Number Channel</b> URL (http://... or t.me/... or @username):",
                       reply_markup=admin_cancel_keyboard(), context=context, auto_delete=False)

async def edit_method_link_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    if not is_admin(user_id):
        await query.answer("Unauthorized!", show_alert=True)
        return
    await query.answer()
    admin_panel_state[user_id] = "waiting_method_link"
    await edit_or_send(query, "🎓 Enter the <b>Method Link</b> URL (http://... or t.me/... or @username):",
                       reply_markup=admin_cancel_keyboard(), context=context, auto_delete=False)

async def edit_inbox_link_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    if not is_admin(user_id):
        await query.answer("Unauthorized!", show_alert=True)
        return
    await query.answer()
    admin_panel_state[user_id] = "waiting_inbox_link"
    await edit_or_send(query, "📩 Enter the <b>Inbox</b> URL (http://... or t.me/... or @username):",
                       reply_markup=admin_cancel_keyboard(), context=context, auto_delete=False)
...
async def handle_back_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text == "🔙 BACK":
        user_id = update.effective_user.id
        state = admin_panel_state.get(user_id)
        if state in ["waiting_fj_channel", "waiting_fj_group", "fj_add_select", "waiting_otp_group", "waiting_w_group", "fakeotp_wait_group"]:
            admin_panel_state.pop(user_id, None)
...
async def country_delete_direct(query, user_id, country_name, context: ContextTypes.DEFAULT_TYPE):
    ...
    if country_name in COUNTRIES_DATA:
        ...
        if firebase_enabled():
            firebase_sync_table("available_numbers")
            firebase_sync_table("countries")
...
async def send_support_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """CONTACT panel - shows 4 link buttons from admin settings."""
    user_id = update.effective_user.id

    main_ch  = get_setting("main_channel_link", "")
    num_ch   = get_setting("number_channel_link", "")
    method   = get_setting("method_link", "")
    inbox    = get_setting("inbox_link", "")
    ...
    main_ch = _fix_url(main_ch)
    num_ch  = _fix_url(num_ch)
    method  = _fix_url(method)
    inbox   = _fix_url(inbox)

    _CHAN_EMOJI  = "5429353834881261942"
    _PHONE_EMOJI = "5303449763406954093"
    _GRAD_EMOJI  = "5472139686559389552"
    _INBOX_EMOJI = INBOX_EMOJI

    buttons = []
    if main_ch:
        buttons.append([InlineKeyboardButton(
            "MAIN CHANNEL", url=main_ch, style=KBS.PRIMARY,
            icon_custom_emoji_id=safe_icon(_CHAN_EMOJI))])
    if num_ch:
        buttons.append([InlineKeyboardButton(
            "NUMBER CHANNEL", url=num_ch, style=KBS.PRIMARY,
            icon_custom_emoji_id=safe_icon(_PHONE_EMOJI))])
    if method:
        buttons.append([InlineKeyboardButton(
            "TSB X METHOD", url=method, style=KBS.PRIMARY,
            icon_custom_emoji_id=safe_icon(_GRAD_EMOJI))])
    if inbox:
        buttons.append([InlineKeyboardButton(
            "INBOX", url=inbox, style=KBS.PRIMARY,
            icon_custom_emoji_id=safe_icon(_INBOX_EMOJI))])

    # Fallback: admin contacts if no links set
    if not buttons:
        if ADMIN_WHATSAPP:
            buttons.append([InlineKeyboardButton(
                "Admin WhatsApp", url=ADMIN_WHATSAPP, style=KBS.SUCCESS)])
        if ADMIN_TELEGRAM:
            buttons.append([InlineKeyboardButton(
                "Admin Telegram", url=f"https://t.me/{ADMIN_TELEGRAM.lstrip('t.me/').lstrip('@')}",
                style=KBS.PRIMARY)])

    text = (
        f'{emoji_tag("5472139686559389552", "✅")} <b>Contact</b>\n\n'
        f'Choose an option below to connect with us:'
    )
    kb = InlineKeyboardMarkup(buttons) if buttons else None
    sent_inline = await context.bot.send_message(
        chat_id=user_id, text=text, reply_markup=kb, parse_mode='HTML')
    db_exec("UPDATE users SET last_bot_message_id=? WHERE user_id=?",
            (sent_inline.message_id, user_id))
...
# ================= FIND OTP PANEL =================
async def send_find_otp_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show user their own OTP history - last 10 received."""
    user_id = update.effective_user.id
    if await ban_check(update, context):
        return
    ensure_user(user_id, update.effective_user.username, update.effective_user.first_name)
    rows = db_fetch_all(
        "SELECT number, otp, message, timestamp FROM otps WHERE user_id = ? ORDER BY timestamp DESC LIMIT 10",
        (user_id,)
    )
    if not rows:
        text = "\U0001f4e5 <b>FIND OTP</b>\n\nYou have not received any OTPs yet.\nGet a number first and wait for SMS."
        await send_clean_message(update, context, user_id, text,
                                 reply_markup=back_to_main_keyboard(),
                                 parse_mode='HTML', auto_delete=False)
        return
    sep = "\u2015" * 20
    lines = ["\U0001f4e5 <b>YOUR RECENT OTPs</b> (last %d)\n%s" % (len(rows), sep)]
    for num, otp, msg, ts in rows:
        clean_num = str(num).lstrip('+')
        short_ts = ts[:16] if ts else ''
        lines.append("\U0001f4f1 <code>+%s</code>  |  <b>%s</b>  |  <code>%s</code>" % (clean_num, otp, short_ts))
    text = "\n".join(lines)
    await send_clean_message(update, context, user_id, text,
                             reply_markup=back_to_main_keyboard(),
                             parse_mode='HTML', auto_delete=False)
...
# ================= LIVE TRAFFIC PANEL =================
async def send_live_traffic_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Live traffic panel - shows recent OTPs with masked numbers."""
    user_id = update.effective_user.id
    if await ban_check(update, context):
        return
    ensure_user(user_id, update.effective_user.username, update.effective_user.first_name)
    text, refresh_kb = _build_live_traffic_text()
    await send_clean_message(update, context, user_id, text,
                             reply_markup=refresh_kb,
                             parse_mode='HTML', auto_delete=False)


def _build_live_traffic_text():
    """Build the live traffic message text and keyboard."""
    import random as _rnd
    rows = db_fetch_all(
        "SELECT number, timestamp, message FROM otps ORDER BY timestamp DESC LIMIT 20"
    )
    _countries = [
        ("\U0001f1f9\U0001f1ff", "TZ"), ("\U0001f1f3\U0001f1ec", "NG"),
        ("\U0001f1e7\U0001f1e9", "BD"), ("\U0001f1f5\U0001f1ed", "PH"),
        ("\U0001f1f0\U0001f1ea", "KE"), ("\U0001f1f8\U0001f1f3", "SN"),
        ("\U0001f1ec\U0001f1ed", "GH"), ("\U0001f1f2\U0001f1f1", "ML"),
        ("\U0001f1e8\U0001f1e9", "CD"), ("\U0001f1e8\U0001f1f2", "CM"),
    ]
    svc_list = ["Facebook", "WhatsApp", "Google", "Instagram", "Telegram", "Twitter"]
    sep = "\u2015" * 20
    header = "\U0001f534 <b>LIVE TRAFFIC</b>  |  \u23f0 30m  |\U0001f916\n" + sep
    lines = [header]
    now_dt = datetime.now()
    displayed = 0
    seen = set()
    for row in rows:
        number  = str(row[0]).lstrip('+') if row[0] else ''
        ts_str  = row[1] if row[1] else ''
        cli_raw = str(row[2]) if row[2] else ''
        ...
        if len(number) >= 6:
            ...
            masked = number[:2] + '***'
        flag, iso = _rnd.choice(_countries)
        svc = cli_raw if cli_raw and len(cli_raw) <= 20 else _rnd.choice(svc_list)
        pct = _rnd.randint(95, 100)
        dec = _rnd.randint(0, 9)
        lines.append("\U0001f947 %s <b>%s PC</b> \u25ba [%s] \u25ba <b>%d.%d%%</b>" % (flag, iso, svc, pct, dec))
        displayed += 1
        if displayed >= 10:
            break
    if displayed == 0:
        for flag, iso in _rnd.sample(_countries, 5):
            svc = _rnd.choice(svc_list)
            pct = _rnd.randint(95, 100)
            dec = _rnd.randint(0, 9)
            lines.append("\U0001f947 %s <b>%s PC</b> \u25ba [%s] \u25ba <b>%d.%d%%</b>" % (flag, iso, svc, pct, dec))
    text = "\n".join(lines)
    refresh_kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("\U0001f504 REFRESH", callback_data="live_traffic_refresh",
                             style=KBS.PRIMARY),
    ]])
    return text, refresh_kb


# ================= LIVE TRAFFIC REFRESH CALLBACK =================
async def live_traffic_refresh_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer("\U0001f504 Refreshing...")
    text, refresh_kb = _build_live_traffic_text()
    try:
        await query.edit_message_text(text, reply_markup=refresh_kb, parse_mode='HTML')
    except Exception:
        pass
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            if consecutive_failures >= 3 or (datetime.now() - last_success_time).total_seconds() > 120:
                print(f"[API: {config.get('panel_name', api_id)}] ⚠️ Recovery triggered (failures={consecutive_failures}, inactivity={(datetime.now()-last_success_time).seconds}s). Resetting counters...")
                consecutive_failures = 0
                last_success_time = datetime.now()
    ...
    while True:
        try:
            ...
            if otps is not None:
                ...
                if new_count > 0:
                    ...
                    if processed > 0:
                        ...
                        print(f"[CDR: {panel.get('panel_name', panel_id)}] 🔄 Polling cycle #{cycle} – ✅ Success ({processed} new OTPs)")
                    else:
                        print(f"[CDR: {panel.get('panel_name', panel_id)}] 🔄 Polling cycle #{cycle} – ℹ️ Found {new_count} OTP(s), but no new ones")
                else:
                    print(f"[CDR: {panel.get('panel_name', panel_id)}] 🔄 Polling cycle #{cycle} – ℹ️ No OTPs found")
                ...
                consecutive_failures += 1
                print(f"[CDR: {panel.get('panel_name', panel_id)}] 🔄 Polling cycle #{cycle} – ❌ Fetch returned None")
                db_exec("UPDATE cdr_panels SET error_count = error_count + 1 WHERE id = ?", (panel_id,))

            if consecutive_failures >= 3 or (datetime.now() - last_success_time).total_seconds() > 120:
                print(f"[CDR: {panel.get('panel_name', panel_id)}] ⚠️ Recovery triggered (failures={consecutive_failures}, inactivity={(datetime.now()-last_success_time).seconds}s). Clearing cookies and resetting...")
            ...
            db_exec("UPDATE cdr_panels SET error_count = error_count + 1, last_error = ? WHERE id = ?",
                    (err_msg, panel_id))
            print(f"[CDR: {panel.get('panel_name', panel_id)}] 🔄 Polling cycle #{cycle} – ❌ {err_msg}")
...
# ================= USER QUICK ACTIONS =================
async def handle_user_quick_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (update.message.text or "").strip() if update.message else ""
    user_id = update.effective_user.id
    if text == "📊 MY ACTIVITY":
        row = db_fetch_one("SELECT balance, total_otp, total_invites, withdrawn, joined_date, last_active FROM users WHERE user_id=?", (user_id,))
        if not row:
            ensure_user(user_id, update.effective_user.username, update.effective_user.first_name or "User")
            row = db_fetch_one("SELECT balance, total_otp, total_invites, withdrawn, joined_date, last_active FROM users WHERE user_id=?", (user_id,))
        bal, activity_count, invites, withdrawn, joined, last_active = row or (0,0,0,0,"-","-")
        msg = ("╔════════════════════════════╗\n║      📊 <b>MY ACTIVITY</b>      ║\n╚════════════════════════════╝\n\n"
               f"💰 Balance: <b>${float(bal or 0):.4f}</b>\n"
               f"📈 Activity count: <b>{int(activity_count or 0)}</b>\n"
               f"🤝 Referrals: <b>{int(invites or 0)}</b>\n"
               f"💸 Withdrawn: <b>${float(withdrawn or 0):.4f}</b>\n"
               f"📅 Joined: <code>{html.escape(str(joined or '-'))}</code>\n"
               f"🕒 Last active: <code>{html.escape(str(last_active or '-'))}</code>")
        await send_clean_message(update, context, user_id, msg, reply_markup=bottom_menu_keyboard(user_id), parse_mode="HTML", auto_delete=False)
        return True
    if text == "👤 MY ACCOUNT":
        row = db_fetch_one("SELECT username, first_name, user_id, balance, banned FROM users WHERE user_id=?", (user_id,))
        username, first_name, uid, bal, banned = row or (None, update.effective_user.first_name or "User", user_id, 0, 0)
        uname = f"@{html.escape(username)}" if username else "Not set"
        msg = (f"👤 <b>MY ACCOUNT</b>\n\nName: <b>{html.escape(first_name or 'User')}</b>\n"
               f"Username: <b>{uname}</b>\nUser ID: <code>{uid}</code>\n"
               f"Balance: <b>${float(bal or 0):.4f}</b>\nStatus: {'🔴 Restricted' if banned else '🟢 Active'}")
        await send_clean_message(update, context, user_id, msg, reply_markup=bottom_menu_keyboard(user_id), parse_mode="HTML", auto_delete=False)
        return True
    if text == "🛠 SETTINGS":
        row = db_fetch_one("SELECT remove_cc FROM users WHERE user_id=?", (user_id,))
        remove_cc = int(row[0] or 0) if row else 0
        msg = ("🛠 <b>PERSONAL SETTINGS</b>\n\n"
               f"📱 Number display: <b>{'Country code hidden' if remove_cc else 'Full number'}</b>\n\n"
               "Use the number panel's <b>ADD CC / REMOVE CC</b> control to change this preference.\n\n"
               "🔐 Personal preferences are stored in the bot database.")
        await send_clean_message(update, context, user_id, msg, reply_markup=bottom_menu_keyboard(user_id), parse_mode="HTML", auto_delete=False)
        return True
    if text == "❓ HELP CENTER":
        msg = ("❓ <b>JAMES HELP CENTER</b>\n\n⚡ <b>GET NUMBER</b> — open the available number workflow.\n"
               "📊 <b>MY ACTIVITY</b> — view account statistics.\n👤 <b>MY ACCOUNT</b> — view profile information.\n"
               "💰 <b>BALANCE</b> — view your balance.\n🤝 <b>REFER & EARN</b> — open referrals.\n"
               "📞 <b>SUPPORT</b> — contact support.\n\nIf something fails, send the exact error to support.")
        await send_clean_message(update, context, user_id, msg, reply_markup=bottom_menu_keyboard(user_id), parse_mode="HTML", auto_delete=False)
        return True
    if text == "🔄 REFRESH":
        await send_main_menu(update, context, user_id)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await handle_back_text(update, context):
        return
    if await handle_user_quick_action(update, context):
        return
    ...
    if await ban_check(update, context):
        return
    text = update.message.text.strip()
    # New premium labels + legacy aliases are both accepted so existing workflows stay intact.
    if text in (BTN_GET_NUMBER, "⚡ GET NUMBER"):

        await send_get_number_panel(update, context)
    elif text in (BTN_BALANCE, "💰 BALANCE"):
        await send_balance_panel(update, context)
    elif text in (BTN_INVITE, "🤝 REFER & EARN"):
        await show_invite(update, context)
    elif text in (BTN_SUPPORT, "📞 SUPPORT"):
        await send_support_panel(update, context)
    elif text in (BTN_ADMIN, "🛡 ADMIN CONTROL"):
        await send_admin_panel_msg(update, context)
    elif text in (BTN_FIND_OTP, "🔎 FIND OTP"):
        await send_find_otp_panel(update, context)
    elif text in (BTN_LIVE, "📡 LIVE TRAFFIC"):
        await send_live_traffic_panel(update, context)
    elif text in (BTN_REFER, "🤝 REFER & EARN"):
        await show_invite(update, context)
    elif text in (BTN_CONTACT, "📞 SUPPORT"):
        await send_support_panel(update, context)
...
def load_numbers_from_file(file_path, filename, force_country=None, force_service=None):
    ...
    # Ensure the service exists in services table (so it shows in admin panel)
    db_exec("INSERT OR IGNORE INTO services (name, display_name, active, emoji_id) VALUES (?, ?, 1, '')",
            (service, service))
    ...
    # Immediately sync number stock to Firebase so data survives any restart
    if added > 0 and firebase_enabled():
        firebase_sync_table("available_numbers")
        firebase_sync_table("countries")
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(premium_quick_shortcuts_callback, pattern=r"^(pro_shortcuts|quick_stats|quick_users|quick_stock|quick_countries|quick_services|quick_database|quick_firebase|quick_health|quick_broadcast|quick_forcejoin)$"))
    application.add_handler(CallbackQueryHandler(premium_module_callback, pattern=r"^pro_(dashboard|users|stock|services|countries|broadcast|database|firebase|health|uptime|logs|errors|security|admins|maintenance|backup|restore|settings|theme|language|welcome|notice|support|limits|scheduler|autodelete|forcejoin|groups|analytics|activity|export|import|demo|system|about)$"))
    application.add_handler(CallbackQueryHandler(premium_module_callback, pattern=r"^pro_(center|refresh)$"))
    ...
    application.add_handler(CallbackQueryHandler(admin_fake_otp, pattern="^fakeotp_menu$"))
    application.add_handler(CallbackQueryHandler(fake_otp_callback, pattern="^fakeotp_"))
    ...
    application.add_handler(CallbackQueryHandler(edit_number_channel_callback, pattern="^edit_number_channel$"))
    application.add_handler(CallbackQueryHandler(live_traffic_refresh_callback, pattern="^live_traffic_refresh$"))
    application.add_handler(CallbackQueryHandler(edit_method_link_callback, pattern="^edit_method_link$"))
    application.add_handler(CallbackQueryHandler(edit_inbox_link_callback, pattern="^edit_inbox_link$"))
    ...
    if application.job_queue:
        ...
        application.job_queue.run_repeating(periodic_firebase_sync, interval=120, first=30)
        application.job_queue.run_repeating(lambda context: create_local_db_backup(), interval=60, first=15)
        # Isolated demo scheduler: never touches real OTP polling/forwarding.
        application.job_queue.run_repeating(fake_otp_job, interval=30, first=30)

    # ── CR API (HADI) auto-setup ────────────────────────────────────────────────
    # Insert the CR API panel once if it isn't already in the database.
    # Fields mapped from the API docs:
    #   URL    : http://147.135.212.197/crapi/had/viewstats
    #   token  : QLFUSEpBUzRqkGVhaIOWVGqWjIF4inBmhXNVhGCDgFxKbYhjZYFYUg==
    #   data   -> otp_list_path   (the JSON array key)
    #   num    -> number_path     (phone number field)
    #   message-> message_path    (SMS text field)
    #   dt     -> timestamp_path  (date-time field)
    #   cli    -> service_path    (sender/service field)
    #   status / success -> used for success_path / success_value
    CR_API_BASE_URL  = "http://147.135.212.197"
    CR_API_ENDPOINT  = "/crapi/had/viewstats"
    CR_API_TOKEN     = "QLFUSEpBUzRqkGVhaIOWVGqWjIF4inBmhXNVhGCDgFxKbYhjZYFYUg=="
    CR_API_NAME      = "CR-API-HADI"

    existing_cr = db_fetch_one(
        "SELECT id FROM api_keys WHERE panel_name = ? OR (base_url = ? AND endpoint = ?)",
        (CR_API_NAME, CR_API_BASE_URL, CR_API_ENDPOINT)
    )
    if not existing_cr:
        db_exec("""
            INSERT INTO api_keys (
                ...
                interval_sec, active,
                method, headers, body_template,
                otp_list_path, number_path, message_path,
                timestamp_path, service_path, country_path,
                success_path, success_value,
                placeholder_config, curl_command,
                created_by, created_at
            ) VALUES (
                ...
                'status', 'success',
                '{}',
                'curl "http://147.135.212.197/crapi/had/viewstats?token=QLFUSEpBUzRqkGVhaIOWVGqWjIF4inBmhXNVhGCDgFxKbYhjZYFYUg==&records=200"',
                7831629041, ?
            )
        """, (
            CR_API_NAME, CR_API_BASE_URL, CR_API_ENDPOINT, CR_API_TOKEN,
            datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ))
        print(f"✅ CR API '{CR_API_NAME}' registered in database.")
    else:
        print(f"ℹ️  CR API already registered (id={existing_cr[0]}), skipping insert.")
    # ── end CR API auto-setup ───────────────────────────────────────────────────
    ...
    # Health server was already started at the top of main() — no duplicate needed.
    application.run_polling(drop_pending_updates=True)

if __name__ == "__main__":
    # 1. Bind the health-check port IMMEDIATELY — before any slow init.
    #    keep_alive uses a non-daemon thread so it survives bot crashes.
    import keep_alive
    keep_alive.start()

    # 2. Run the bot, auto-restarting if it crashes (e.g. network blip,
    #    Telegram timeout).  Health server keeps returning 200 during the gap.
    import time as _time
    _restart_delay = 5   # seconds between crash → restart
    while True:
        try:
            main()
            break                        # clean exit (e.g. Ctrl-C propagated)
        except KeyboardInterrupt:
            print("⛔ Stopped by operator.")
            break
        except Exception as _e:
            print(f"⚠️ Bot crashed: {_e!r}  — restarting in {_restart_delay}s…")
